...
1.  **Получение IP-адреса**: Скрипт обращается к внешнему сервису, чтобы автоматически определить **публичный IP-адрес** вашего сервера. Этот IP используется как основное имя для сертификата. Это удобно, если у вас нет доменного имени.
2.  **Генерация ключей**: Затем скрипт создает **приватный ключ** (`secret.key`). Этот ключ является секретным и используется для подписи сертификата. Он похож на ваш личный отпечаток. 
3.  **Создание сертификата**: Наконец, скрипт использует приватный ключ для создания **самоподписанного сертификата** (`cert.crt`). Этот сертификат содержит ваш IP-адрес и другую информацию, подтверждая, что именно ваш сервер обеспечивает безопасное соединение. Он действует 10 лет, так что вам не придется часто его обновлять.
...
Для удобства и порядка рекомендуется создать отдельную папку для файлов сертификата и ключа. Это поможет избежать беспорядка в вашей домашней директории.

1.  **Создайте новую папку:** Выполните команду `mkdir <имя_папки>`. Например, `mkdir certs`.
2.  **Перейдите в эту папку:** Используйте команду `cd <имя_папки>`, например, `cd certs`.

**Или сделай проще (для создания дирректории и входа в неё), копируй следующие совмещённые команды**

---

### **Объединение команд**

Для того чтобы создать папку и сразу в неё зайти, можно использовать логический оператор `&&`. Эта команда выполнится только в том случае, если первая команда (`mkdir`) завершится без ошибок.
...
**Как это работает:**
  ...
  * `mkdir certs` - создаёт папку с именем `certs`.
  * `&&` - оператор "и". Если `mkdir` успешно завершится, то выполнится следующая команда.
  * `cd certs` - переходит в только что созданную папку.

---

Теперь вы находитесь в созданной папке, и можете запустить скрипт. Скрипт создаст файлы `cert.crt` и `secret.key` именно здесь, и они не будут мешать другим вашим файлам.
...
```bash
wget -qO- https://raw.githubusercontent.com/Svodnet/3x-ui-cert-generator/refs/heads/main/generate-cert.sh | bash
```

**Для локальной (домашней) виртуалки или домашнего отдельного ПК использованного в качестве сервера:**

```bash
wget -qO- https://raw.githubusercontent.com/Svodnet/3x-ui-cert-generator/refs/heads/main/generate-cert-local.sh | bash
```
